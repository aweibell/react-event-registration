...
import ShowRowHeaders from './ShowRowHeaders';
...
class ShowCollection extends Component {
  ...
  editRow(rowIndex) {
    // TODO: Implement
    alert('not supported yet' + rowIndex);
  }
  ...
  render() {
    ...
    return (
      <div style={collectionStyle}>
        ...
        <table>
          <thead>
          <ShowRowHeaders columns={columns} collectionId={id} />
          </thead>
          <tbody>
        {
          this.props.collection.map((data, index) => {
            return (
              <ShowRow key={index} index={index} collectionId={id} columns={columns} data={data}
